...
__global__ void rpg3ker(double *sr1, double *sr2, double *sr3, long long *srCln, long long *Nsr, long long *Ncol,
                        long long *idx, long long *ttl,double *sn1, double *sn2, double *sn3, long long *snCln,
    ...
    for (j=0; j < *Nsr; j++) // loop through all source pixel indicies
    {
      if (idx[j]==mytarget) // if the source pixel index matches our target
      {
        for (k=0; k < *Ncol; k++) //loop over the columns for that pixel and
        {
          ...
          sn3[l]+=sr3[m]; // add to the target sink3 pixel columns
...
extern "C" void rpg3(double *sr1, double *sr2, double *sr3, long long *srCln, long long Nsr, long long Ncol,
          long long *idx, long long *ttl, double *sn1, double *sn2, double *sn3, long long *snCln, long long Nsn,
          long long *snTar, long long Ntar)
{
  double *d_sr1, *d_sr2, *d_sr3, *d_sn1, *d_sn2, *d_sn3;
  ...
  cudaMalloc( (void**)&d_sr3, Nsr*Ncol*sized);
  cudaMalloc( (void**)&d_sn1, Nsn*Ncol*sized);
  cudaMalloc( (void**)&d_sn2, Nsn*Ncol*sized);
  cudaMalloc( (void**)&d_sn3, Nsn*Ncol*sized);
  ...
  cudaMemcpy( d_sr3,   sr3,   Nsr*Ncol*sized, cudaMemcpyHostToDevice);
  cudaMemcpy( d_sn1,   sn1,   Nsn*Ncol*sized, cudaMemcpyHostToDevice);
  cudaMemcpy( d_sn2,   sn2,   Nsn*Ncol*sized, cudaMemcpyHostToDevice);
  cudaMemcpy( d_sn3,   sn3,   Nsn*Ncol*sized, cudaMemcpyHostToDevice);
  ...
  rpg3ker<<<Nblocks,Nthreads>>>(d_sr1,d_sr2,d_sr3,d_srCln,d_Nsr,d_Ncol,d_idx,d_ttl,d_sn1,d_sn2,d_sn3,d_snCln,d_snTar,d_Ntar);

  cudaDeviceSynchronize();

  cudaMemcpy( sn1, d_sn1, Nsn*Ncol*sized, cudaMemcpyDeviceToHost);
  cudaMemcpy( sn2, d_sn2, Nsn*Ncol*sized, cudaMemcpyDeviceToHost);
  cudaMemcpy( sn3, d_sn3, Nsn*Ncol*sized, cudaMemcpyDeviceToHost);
