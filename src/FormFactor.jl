...
#
#function spinstructurefactor(
#        ion::Vector{S},
#        spin::Vector{LDVec{N,T}},
#        d::Vector{LDVec{N,T}},
#        occupancy::Vector{T},
#        q::LQVec{N,R},
#        g::Real=2) where {N,T<:Real,R<:Real,S<:AbstractString}
#    @assert length(ion)==length(spin)==length(d)==length(occupancy)
#    qhat = q/norm(q)
#    ok = norm.(spin) .> 0
#    ion=ion[ok]
#    s=spin[ok]
...
#    o=occupancy[ok]
#    @assert all(0.<=o.<=1)
#    F=sum(o.*formfactor.(q,ion,g).*exp.(-im*dot.(q,d)).*cross.(qhat,cross(s,qhat)))
#    return F*0.26954201169126937*g # where the constant is γrₒ/2 in units of sqrt(barns) [10^-14 m]
#end
#function spinstructurefactor(
#        ion::Vector{S},
#        spin::Vector{LDVec{N,T}},
...
#    o=occupancy[ok]
#    @assert all(0.<=o.<=1)
#    F=zeros(Complex{Base.promote_op(*,T,R)},size(q))
#    for i=1:length(q)
#        F[i]=sum(o.*formfactor.(q[i],ion,g).*exp(-im*dot(q[i],d)).*cross.(qhat[i],cross.(s,qhat[i])))
#    end
#    return F*0.26954201169126937*g # where the constant is γrₒ/2 in units of sqrt(barns) [10^-14 m]
#end
...
"""
    elementsplit(::String)
Take a string possibly containing an atomic number, element symbol,
and ionization state, and return the tuple of the three with empty
strings for missing items.
E.g., elementsplit("55Fe2+") returns ("55","Fe","2+")
"""
function elementsplit(aei::AbstractString)
    relement=r"[a-z,A-Z]+"
...
"""
    formfactor(q,ion,g)
returns the dipole magnetic form factor, `F(q)`, for the momentum transfer magnitude, `q`,
atom with ionization state, `ion`, and Lánde g-factor, `g` (`g=2` for spin-only).

The passed value of `ion` can contain Atomic Number, Element Symbol, and Ionization State 
information of the form `"55Fe2+"` or just Element Symbol and Ionization State, e.g., `"Fe2+"`.
Since the magnetic form factor is isotope independent, any Atomic Number information is discarded.
    
In the dipole approximation `F = <j₀(s)> + (1-2/g)<j₂(s)>` where `s=sin(θ)/λ`.
The `<jᵢ(s)>` are bare form factors given by

    <jᵢ(k)>= ∫₀ⁱⁿᶠⁱⁿᵗʸ U²(r) jᵢ(kr) 4πr² dr

in which `U(r)` is the radial wavefunction for the unpaired electrons in the atom, 
`k` is the length of the scattering vector, and `jᵢ(kr)` is the `i`th spherical Bessel function.

Measurements of `<jᵢ(s)>` have been fit to an approximating function of the form

    <j₀(s)>=A exp(-a s²) + B exp(-b s²) + C exp(-c s²) + D
    
    <jᵢ(s)>=A s² exp(-a s²) + B s² exp(-b s²) + C s² exp(-c s²) + D s² [i=2,4,6]

with tabulated parameters `[A,a,B,b,C,c,D,e]` given in the 
International Tables of Crystallography (2006), Vol. C, Chapter 4.4, Section 4.4.5
by P.J. Brown, pg 454-461, tables 4.4.5.1 through 4.4.5.14.
The last parameter, `e`, is a measure of the quality of the approximation's fit to
the experimental observations (see the text for details).

The tabulated expressions are expressed in terms of `s=sin(θ)/λ`, but we prefer here to
work with `q= 4π sin(θ)/λ`, so `formfactor` internally calculates `s` from `q`.
"""
function formfactor(q,anelis::AbstractString,g::Real=2)
    (AN,El,Is)=elementsplit(anelis) # we don't care about the isotope specification here
    ion=El*Is
    s2 = (q/4/pi).^2;
    ff=_jn(s2,ion,0) 
    g!=2 && (ff+=(1-2/g)*_jn(s2,ion,2) )
    return ff
end
function formfactor(qv::Array,a::Vector{T},g::Real=2) where T<:AbstractString
    # for the q-points specified in qv, calculate the form factor for each of the ions in a
    # the size(output) == (size(qv),length(a))
    ff1=formfactor(qv,a[1],g)
    n=length(a)
    out=Array{eltype(ff1)}(n,size(qv)...) # ideally, (size(qv)...,n) but indexing the last index is tough
    out[1,:]=ff1[:]
    for i=2:n
        out[i,:]=formfactor(qv,a[i],g)[:]
    end
    return permutedims(out,circshift(1:ndims(out),-1)) # maybe slow, but move from (n,size(qv)...) to (size(qv)...,n)
end
# for other Array input just use map ¯\_(ツ)_/¯
formfactor(q,a::Array,g::Real=2)=map(x->formfactor(q,x,g),a)

    
function _jn{T<:Integer}(s2,ion::AbstractString,n::T=0)
    c=_jn_coefficients(ion,n)
    jn=c[1]*exp.(-c[2]*s2)+c[3]*exp.(-c[4]*s2)+c[5]*exp.(-c[6]*s2)+c[7]
    n>0 && (jn.*=s2)
    return jn
end
function _jn_coefficients{T<:Integer}(ion::AbstractString,n::T=0)
    @assert iseven(n)&(0<=n<=6) "only j_n coefficients for n=0,2,4,6 are tabulated"
    fallback=[1.,0,0,0,0,0,0,-1]
    jn_dict=get(_jn_coefficient_dict,n,fallback)
    if isa(jn_dict,Dict)
        c=get(jn_dict,ion,fallback)
        all(c.==fallback) && warn("formfactor: Unknown ion $ion; returned j$n is unity for all q")
    else
        all(jn_dict.==fallback)?warn("formfactor: unkown jₙ"):error("formfactor: problem accessing jn coefficients")
        c=jn_dict
    end
    return c
end
#
# The following works but doesn't appear to provide a speed benefit
#
#function _jn{T<:Integer}(s,ion::AbstractString,n::T=0)
#    c=_jcouter(ion,n)
#    jn=c[1]*exp(-c[2]*s)+c[3]*exp(-c[4]*s)+c[5]*exp(-c[6]*s)+c[7]
#    n>0 && (jn.*=s)
#    return jn
#end
## This is probably a bad idea (using a array size specification to pick-out differen jNs, but it works
#_jcouter(ion::AbstractString,n::Integer)=_jcinner(ion,ones(Int64,ones(Int64,n)...))
#_jcinner{N}(ion::AbstractString,::Array{Int64,N}=ones(Int64,ones(Int64,N)...))=get(include("j$N.dict"),ion,[1,0,0,0,0,0,0,0,-1.])

const _jn_coefficient_dict = Dict(
(
# j0 dictionary from Tables 4.4.5.1 -- 4.4.5.4 in 
# the International Tables of Crystallography (2006), 
# Vol. C, Chapter 4.4, Section 4.4.5, by P.J. Brown, pg 454-461.
# ion         A         a        B        b        C         c        D        e(rror)
(0,
Dict( (
...
# j2 dictionary from Tables 4.4.5.5 -- 4.4.5.8 in 
# the International Tables of Crystallography (2006), 
# Vol. C, Chapter 4.4, Section 4.4.5, by P.J. Brown, pg 454-461.
# ion         A         a        B        b        C         c        D        e(rror)
(2,
...
# j4 dictionary from Tables 4.4.5.9 -- 4.4.5.12 in 
# the International Tables of Crystallography (2006), 
# Vol. C, Chapter 4.4, Section 4.4.5, by P.J. Brown, pg 454-461.
# ion         A         a        B        b        C         c        D        e(rror)
(4,
...
# j6 dictionary from Tables 4.4.5.13 -- 4.4.5.14 in 
# the International Tables of Crystallography (2006), 
# Vol. C, Chapter 4.4, Section 4.4.5, by P.J. Brown, pg 454-461.
# ion         A         a        B        b        C         c        D        e(rror)
(6,
...
) # end of Tuple{Integer,Dict{AbstractString,Array{Float64,1}}}
) # end of Dict call
