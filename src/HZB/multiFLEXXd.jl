...
type multiFLEXXd <: TripleAxisd{2}
    ...
    function multiFLEXXd(fn::AbstractString,cmd::AbstractString,hd::Vector{S1},
                       ...
                       instdef::Function=defineFLEXX) where {S1<:AbstractString,
                                                             S2<:AbstractString,
                                                             S3<:AbstractString,
                                                             S4<:AbstractString,
                                                             S5<:AbstractString,
    ...
    multiFLEXXd()=new()
    multiFLEXXd(d::multiFLEXXd)=deepcopy(d)
end
...
function fancy_hkl(h::Real,k::Real,l::Real;H="H",K="K",L="L")
    fh=fancy_int(h,H)
...
function applycorrections!(a::multiFLEXXd;int_corr_csv::AbstractString="",weights_csv::AbstractString="",alive_csv::AbstractString="")
    raw=getDat(a,"intensity")
    (npt,na4,nen)=size(raw) # if multiFLEXX na4==31, nen==5
...
#function applycorrections!(a::multiFLEXXd,int_corr_csv::AbstractString,weights_csv::AbstractString,alive_csv::AbstractString)
#    @assert isfile(int_corr_csv) && isfile(weights_csv) && isfile(alive_csv)
#    raw=getDat(a,"intensity")
#    (npt,na4,nen)=size(raw) # if multiFLEXX na4==31, nen==5
#
#    int_corr=readdlm(int_corr_csv,',')
...
"""
    flat2multiFLEXX(columns,data,flat)
The ILL TAS data format was designed for 0D detector instruments while multiFLEXX has 5 1D detectors.
To shoehorn multiFLEXX data into a ILL TAS data file some (constant) information is discarded and all detector pixels are stored in a single "point".
This works for data storage but is undesirable for data visualization and manipulation.
This function transforms the 2D ILL TAS data block into a 3D multiFLEXX data block restoring information which was stripped.
"""
function flat2multiFLEXX(cols::Vector{C},data::Matrix{T},flat::Matrix{T}) where {T,C}
    na4=31
    nen=5
    @assert size(flat,2) == (na4+1)*nen "each row should have $((na4+1)*nen) values for multiFLEXX"
    npt=size(flat,1)
    if size(data,1)!=npt
    ...
    # TODO: move [a5,a6] calculations to definemultiFLEXX since they're constants
    τs=gettau("PG(002)")
    Efs=(5:9)/2
...
# multiFLEXX is a CAMEA-detector triple-axis neutron diffractometer at HZB
# multiFLEXXd0 holds data from a multiFLEXXd object in a single data block, this
...
type multiFLEXXd0 <: TripleAxisd{0}
    filename::AbstractString
    command::AbstractString
    header::Array{AbstractString,1}
    ...
    multiFLEXXd0()=new()
    multiFLEXXd0(d::multiFLEXXd0)=deepcopy(d)
end
zerotype(::multiFLEXXd)=multiFLEXXd0
